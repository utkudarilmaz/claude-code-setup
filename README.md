...
1. Authentication & Authorization (auth flows, session management, access control, MFA, JWT)
2. Input Validation (SQL injection, XSS, command injection, path traversal, XXE, LDAP, template injection)
3. Data Exposure (PII in logs, sensitive data in URLs, encryption, TLS, stack traces)
4. Secrets Management (hardcoded credentials, API keys, tokens, version control)
5. OWASP Top 10 (2021) - A01 through A10 with cross-references
6. API Security (rate limiting, mass assignment, GraphQL, CORS, OAuth/OIDC)
7. File Upload Security (type validation, size limits, malicious content, polyglot files, ZIP bombs)
8. Cryptography (hashing, salts, key derivation, weak ciphers, TLS versions, timing attacks)
9. Business Logic Vulnerabilities (race conditions, workflow bypass, integer overflow, replay attacks)
10. Client-Side Security (localStorage tokens, postMessage validation, clickjacking, prototype pollution)
11. HTTP Security Headers & Cookies (CSP, HSTS, X-Frame-Options, Secure/HttpOnly/SameSite)
12. Dependency Security (CVEs, outdated packages, supply chain risks, lockfile integrity)
13. Modern Attack Vectors (prototype pollution, ReDoS, request smuggling, WebSocket, SSRF, deserialization)

**Severity Levels:** CRITICAL, HIGH, MEDIUM, LOW

**Language Coverage:** Python, Go (with language-specific dangerous patterns)
